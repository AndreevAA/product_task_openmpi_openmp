...
Параллелизация цикла для обновления расстояний:
Добавлена директива #pragma omp parallel for, которая позволяет распараллелить обновление расстояний соседних вершин. Это возможно, так как каждый поток будет работать со своим набором данных.
Параллелизация цикла в функции solveProductSubproblem:
Подобным образом добавлена директива #pragma omp parallel for перед циклом, который выполняет Dijkstra для каждой вершины, что позволяет многопоточному выполнению алгоритма Dijkstra.
Критическая секция:
Для предотвращения конфликтов, связанных с выводом в стандартный поток, добавлена директива #pragma omp critical, чтобы обеспечить, что только один поток выполняет вывод информации в одно и то же время.
*/
...
void dijkstra(int start_vertex, int numVertices, Edge *edges, int numEdges, int *distances) {
    ...
    for (int i = 0; i < numVertices - 1; i++) {
        ...
        // Параллельная секция для обновления расстояний соседних вершин
        #pragma omp parallel for
...
void solveProductSubproblem(int productId, int numVertices, int numEdges, 
    ...
    // Параллельный цикл по всем вершинам
    #pragma omp parallel for
    for (int i = 0; i < numVertices; i++) {
